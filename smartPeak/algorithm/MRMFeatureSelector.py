...
class MRMFeatureSelector():
    ...
    def select_MRMFeatures_qmip(
        ...
        Args:
            ...
            select_criteria (list,dict): e.g., [{"name":, "value":, }]
                ...
                name: "locality_weights": value:True, type: boolean, description: 
                    weight compounds with a nearer Tr greater 
                    than compounds with a further Tr
                name: "select_transition_groups": value:True, type: boolean, description:
                    select transition groups or transitions
                name: "segment_window_length": value:12, type:
                    float, description: 
        ...
        for feature in features:
            ...
            feature_metaValues = {
                k.decode('utf-8'): feature.getMetaValue(k) for k in keys}
            if select_transition_groups:   
                tmp = {  # note component_name ~ component_group_name
                ...
                if component_group_name not in Tr_dict.keys():
                ...
                Tr_expected_dict[component_group_name] = {
                    ...
                    # note component_name ~ component_group_name
                    'component_name': component_group_name, 
                    'component_group_name': component_group_name,
                    }
            else:
                for subordinate in feature.getSubordinates():
                    ...
                    subordinate_metaValues = {
                        k.decode('utf-8'): subordinate.getMetaValue(k) for k in keys}
                    tmp = {
                        'component_group_name': component_group_name, 
                    ...
                    if component_name not in Tr_dict.keys():
        ...
        for i in range(segments):  # could be distributed and parallelized
            ...
            if select_highest_count:
                for var in tmp:
                    ...
                    if var not in Tr_optimal_count.keys():
        ...
        if select_highest_count:
            ...
            for var, count in Tr_optimal_count.items():
                ...
                transition_id = var.split('_')[-1]
                if var not in Tr_optimal_dict.keys():
        ...
        for feature in features:
            ...
            for subordinate in feature.getSubordinates():
                if select_transition_groups:
                    var_name = "%s_%s" % (
                        feature.getMetaValue("PeptideRef").decode('utf-8'),
                        feature.getUniqueId()) 
                else:
                    var_name = "%s_%s" % (
                        subordinate.getMetaValue("native_id").decode('utf-8'),
                        feature.getUniqueId())                
    ...
    def optimize_Tr(
        ...
        Args:
            ...
            locality_weights (boolean): weight compounds with a nearer Tr greater 
                than compounds with a further Tr
            variable_type (str): the type of variable, 'integer' or 'continuous'
            optimal_threshold (float): value above which the transition group or 
                transition is considered optimal (0 < x < 1)
        ...
        Potential Speed Optimizations
            Add in a check to see if multiple peaks for a transition or transition group 
            actually exist
        ...
        for cnt_1, v1 in enumerate(To_list):
            if verbose_I:
                print("""Building and adding variables and constraints
            ...
            for i_1, row_1 in enumerate(Tr_dict[component_name_1]):
                ...
                if score_weights and score_weights is not None:
                    score_1 = pow(score_1, 1/len(score_weights))
                ...
                # NOTE: +1 to compensate for component_name_1 != component_name_2
                stop_iter = min([cnt_1 + nn_threshold + 1, len(To_list)])  
                for cnt_2, v2 in enumerate(To_list[start_iter:stop_iter]):
                    ...
                    if component_name_1 == component_name_2:
                        continue
                    if component_name_2 not in Tr_dict.keys():
                        continue
                    for i_2, row_2 in enumerate(Tr_dict[component_name_2]):
                        ...
                        if variable_name_2 not in variables.keys():
                            ...
                            n_variables += 1
                        score_2 = self.make_score(
                            Tr_dict[component_name_2][i_2], score_weights)
                        if score_weights and score_weights is not None:
                            score_2 = pow(score_2, 1/len(score_weights))
                        ...
                        try:
                            model.add(Constraint(
                                ...
                                lb=0))
                            model.constraints[
                                obj_constraint_name+'-QP1'].set_linear_coefficients({
                        ...
                        try:
                            model.add(Constraint(
                                ...
                                lb=0))
                            model.constraints[
                                obj_constraint_name+'-QP2'].set_linear_coefficients({
                        ...
                        model.add(Constraint(
                            ...
                            ub=1))
                        model.constraints[
                            obj_constraint_name+'-QP3'].set_linear_coefficients({
                                variables[variable_name_1]: 1,
                                variables[variable_name_2]: 1, var_qp: -1
                                })
                        ...
                        try:
                            model.add(Constraint(
                                ...
                                ub=0))
                            model.constraints[
                                obj_constraint_name+'-obj+'].set_linear_coefficients({
                                    obj_variables[obj_variable_name]: -1, 
                                    var_qp: locality_weight*score_1*score_2*(
                                        tr_delta-tr_delta_expected)
                                        })
                        except Exception as e:
                            print(e)
                        # model.add(Constraint(
            ...
            try:
                ...
                model.constraints[constraint_name_1].set_linear_coefficients({
                    d: 1 for d in constraints})
                n_constraints += 1    
            except Exception as e:
                print(e)
        if verbose_I:
        ...
        # st = time.time()
        # model.add([
        #   Constraint(sum(v),name=constraint_name, lb=1, ub=1)
        #   for constraint_name, v in constraints.items()])
        # # for constraint_name, v in constraints.items():
        # #     model.add(Constraint(sum(v),name=constraint_name, lb=1, ub=1))
        # for constraint_name, v in obj_constraints.items(): #model.add([
        #   v for constraint_name, v in obj_constraints.items()])
        #     model.add(v)
        # make the objective
        ...
        if verbose_I: 
            print("Elapsed time: %.2fs" % elapsed_time)
        Tr_optimal = [
            var.name for var in model.variables if var.primal > 
            optimal_threshold and var.name in variables.keys()]
        # # DEBUGING:
        # print(len(Tr_optimal),len(variables.keys()))
        # Tr_primals = [
        #   {var.name:var.primal} for var in model.variables 
        #   if var.name in variables.keys()]
    ...
    def schedule_MRMFeatures_qmip(
        ...
        for i in range(len(segment_window_lengths)):
            if verbose_I: 
                print("Optimizing for iteration (%s/%s)" % (
                    i, len(segment_window_lengths)-1))
            select_criteria = [
                {"name": "nn_threshold", "value": nn_thresholds[i]},
                {"name": "locality_weights", "value": locality_weights[i]},
                {
                    "name": "select_transition_groups", 
                    "value": select_transition_groups[i]},
    ...
    def optimize_scores(
        ...
        Args:
            ...
            optimal_threshold (float): 
                value above which the transition group or 
                transition is considered optimal (0 < x < 1)
        ...
        for cnt_1, v1 in enumerate(To_list):
            if verbose_I: 
                print("""Building and adding variables and 
