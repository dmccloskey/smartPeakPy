...
class DescriptiveStatistics():
    ...
    def calculate_descriptiveStats(
        self,
        data_I,
        confidence_I=0.95,
        iq_range_I=[25, 75]
        ...
        try:
            mean, var, cv, lb, ub = self.calculate_ave_var_cv(
                data_I, confidence_I=confidence_I)
            min, max, median, iq_1, iq_3 = self.calculate_interquartiles(
                data_I, iq_range_I=iq_range_I)
            descriptiveStats_O = {
                'n': len(data_I),
                'mean': mean,
                'var': var,
